...
| `test_replay_normal_vitals_hold` | SpO2=100（正常）→ HOLD（初回発行あり） |
| `test_replay_no_data_guard_overrides_critical_spo2` | age_sec=15 > 10 → 常に HOLD |
| `test_replay_age_sec_over_threshold_is_no_data` | age_sec=10.001 > no_data=10 → HOLD |
...
| `test_replay_no_alert_no_abnormal_vitals_hold` | alert なし・正常 SpO2 → HOLD |
...
> `workflows` 権限制限により本 PR の対象外。メンテナーが別途対応する。
...
- `colcon test --packages-select medical_robot_sim` が成功する（新規 14 テスト含む）
