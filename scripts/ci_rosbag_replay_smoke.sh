#!/usr/bin/env bash
# CI smoke test: record vitals to rosbag, replay them, and verify alerts.
#
...
# Avoid the ros2 CLI daemon in headless CI, matching the repository's
# acceptance guidance for topic-related commands.
...
record_bag() {
  echo "Recording vitals bag in ${BAG_DIR}"
  rm -rf "${BAG_DIR}"
  ...
  # Let the recorder capture live samples before injecting a deterministic
  # abnormal sample that should produce a replay alert.
  sleep 2
  publish_abnormal_vitals_once
  sleep 3
...
replay_bag_and_expect_alert() {
  echo "Replaying ${BAG_DIR} through icu_replay.launch.py"
...
main() {
  ...
  echo "CI rosbag replay smoke test passed"
}
